...
Define(ferocious_bite 67599)
Define(healing_touch 54869)
...
Define(shred 54859)
Define(skull_bash_cat 106839)
  SpellInfo(skull_bash_cat cd=15 )
  SpellAddBuff(skull_bash_cat skull_bash_cat=1)
...
AddIcon mastery=2 help=main
{
	...
	if target.DebuffPresent(rip) and target.DebuffRemains(rip) <=4 Spell(ravage)
	...
	if BuffPresent(omen_of_clarity) Spell(ravage)
	if {BuffPresent(tigers_fury) or BuffPresent(berserk) } Spell(ravage)
	if {{ComboPoints() <5 and target.DebuffRemains(rip) <3.0 } or {ComboPoints() ==0 and BuffRemains(savage_roar) <2 } } Spell(ravage)
	if SpellCooldown(tigers_fury) <=3.0 Spell(ravage)
	if target.DeadIn() <=8.5 Spell(ravage)
	if TimeToMaxEnergy() <=1.0 Spell(ravage)
}
