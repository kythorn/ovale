...
Define(blood_plague 55078)
  SpellInfo(blood_plague duration=30 )
  SpellAddBuff(blood_plague blood_plague=1)
...
Define(frost_fever 55095)
  SpellInfo(frost_fever duration=30 )
  SpellAddBuff(frost_fever frost_fever=1)
...
AddIcon mastery=2 help=main
{
	...
	if TalentPoints(plague_leech) and {{SpellCooldown(outbreak) <1 } or {BuffPresent(rime) and target.DebuffRemains(blood_plague) <3 and {RuneCount(unholy) >=1 or RuneCount(death) >=1 } } } Spell(plague_leech)
...
AddIcon mastery=2 help=offgcd
{
	...
	if TalentPoints(blood_tap) Spell(blood_tap)
}
...
AddIcon mastery=3 help=main
{
	...
	if not target.DebuffPresent(blood_plague) Spell(plague_strike)
	if TalentPoints(plague_leech) and {SpellCooldown(outbreak) <1 } Spell(plague_leech)
